...
#'
#' \code{read_6800} Reads LI-6800 excel data files, which are delimited by tabs.
#'
#' @param filename path and name of your data.
#' @param data_start the start of your data(without headline)
#' @return read_6800 imports a LI-6800 excel data file as a data frame
...
readxl_6800 <- function(filename, data_start = 41){
  
  # only read measured data -------------------------------------------------
  
  data <- read_excel(filename, skip = data_start-2, col_names = TRUE)
